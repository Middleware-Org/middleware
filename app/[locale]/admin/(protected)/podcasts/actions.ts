...
export async function createPodcastAction(
  ...
  try {
    ...
    const audio_chunks = formData.get("audio_chunks") as string;
    const duration = formData.get("duration") as string | null;
    const published = formData.get("published") === "true";
    const date = formData.get("date") as string;
    const slug = formData.get("slug") as string | null;

    if (!title || !audio || !audio_chunks || !date) {
      return {
        success: false,
        error: "Title, audio, audio chunks and date are required",
        errorType: "error",
      };
    }
...
export async function updatePodcastAction(
  ...
  try {
    ...
    if (!slug || !title || !audio || !audio_chunks || !date) {
      return {
        success: false,
        error: "Slug, title, audio, audio chunks and date are required",
        errorType: "error",
      };
    }
...
export async function deletePodcastsAction(
  ...
  try {
    ...
    if (failed > 0) {
      return {
        success: false,
        error: `Eliminati ${deleted} podcast, ${failed} falliti. ${errors.join("; ")}`,
        errorType: "warning",
      };
    }
    ...
    return {
      success: true,
      data: { deleted, failed },
      message: `Eliminati con successo ${deleted} podcast`,
    };
  } catch (error) {
    return {
      success: false,
