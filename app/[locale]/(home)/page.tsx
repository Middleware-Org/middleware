...
export default async function RootPage({ params }: RootPageProps) {
  ...
  return (
    <div className="max-w-[1472px] mx-auto px-0 md:px-4 lg:px-10 py-0 md:py-6 lg:py-10">
      ...
      <HorizontalScroll stickyOffset={155} className="gap-10 mb-10">
        {Array.from({ length: 50 }).map((_, i) => (
          <div
            key={i}
            style={{ marginLeft: `${i === 0 ? 0 : 20}px`, height: `calc(100vh - 195px)` }}
            className="w-[40vw] rounded-lg bg-emerald-500 flex items-center justify-center text-xl font-semibold text-white shrink-0"
          >
            Card {i + 1}
          </div>
        ))}
