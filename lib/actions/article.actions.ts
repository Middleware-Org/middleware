/**
 * Article Actions
 * Server Actions per la gestione degli articoli con invalidazione cache
 */
'use server';
...
import { ARTICLE_CACHE_TAGS } from '@/lib/services/article.service';
...
export async function updateArticleAction(
  ...
  try {
    ...
    revalidateTag(ARTICLE_CACHE_TAGS.detail(slug));
    if (newSlug) revalidateTag(ARTICLE_CACHE_TAGS.detail(newSlug));
    revalidateTag(ARTICLE_CACHE_TAGS.all);
    revalidatePath('/admin/articles');
    revalidatePath('/[locale]/articles', 'page');
...
export async function deleteArticlesAction(slugs: string[]): Promise<ActionResult<{ deleted: number; failed: number }>> {
  try {
    const user = await getUser();
    if (!user) {
      return { success: false, error: 'Unauthorized', errorType: 'error' };
    ...
    if (!slugs || slugs.length === 0) {
      return { success: false, error: 'No articles selected', errorType: 'error' };
    }
    ...
    if (failed > 0) {
      return {
        success: false,
        error: `Deleted ${deleted} articles, failed to delete ${failed}`,
        errorType: 'warning',
      };
    }
    ...
    return {
      success: true,
      data: { deleted, failed },
      message: `Successfully deleted ${deleted} articles`,
    };
  } catch (error) {
    return {
      success: false,
