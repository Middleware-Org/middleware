/**
 * Podcast Actions
 * Server Actions per la gestione dei podcast con invalidazione cache
 */
'use server';
...
import { PODCAST_CACHE_TAGS } from '@/lib/services/podcast.service';
...
export async function createPodcastAction(
  ...
  try {
    ...
    revalidatePath('/[locale]/podcasts', 'page');
...
export async function updatePodcastAction(
  ...
  try {
    ...
    revalidateTag(PODCAST_CACHE_TAGS.detail(slug));
    if (newSlug) revalidateTag(PODCAST_CACHE_TAGS.detail(newSlug));
    revalidateTag(PODCAST_CACHE_TAGS.all);
    revalidatePath('/admin/podcasts');
    revalidatePath('/[locale]/podcasts', 'page');
